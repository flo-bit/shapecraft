<script lang="ts">
	...
	let {
		...
		showGrid,
		wireframe
	}: { model: THREE.Mesh | null; viewDistance: number; showGrid: boolean; wireframe: boolean } =
		$props();
	...
	$effect(() => {
		if (model) {
			(model.material as THREE.MeshStandardMaterial).wireframe = wireframe;
			invalidate();
		}
	});
	scene.background = new THREE.Color(0x8eb3d9);
	scene.fog = new THREE.FogExp2(0x8eb3d9, 0.022);
...
<T.HemisphereLight args={[0x87ceeb, 0x3a5a2a, 0.6]} />
<T.DirectionalLight
	position={[8, 12, 5]}
	intensity={1.4}
	color={0xfff4e0}
...
{#if showGrid}
	<T.GridHelper args={[30, 30, 0xe8f0f8, 0xc4d2de]} position.y={0.32} />
{/if}
